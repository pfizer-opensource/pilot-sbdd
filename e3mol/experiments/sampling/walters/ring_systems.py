# Taken from https://github.com/Dunni3/FlowMol/blob/main/flowmol/analysis/ring_systems.py
...
class RingSystemCounter:
    ...
    def count_ring_systems(
        ...
        """
        sample_counts: defaultdict = defaultdict(int)
        chembl_counts = {}
        n_mols = len(rdmols)
        for mol in rdmols:
    ...
    def combine_counts(
        self, counts_list: List[Tuple[Dict[str, int], Dict[str, int], int]]
    ) -> Tuple[Dict[str, int], Dict[str, int], int]:
        """
        ...
        Returns two dictionaries: one with the combined counts
        of ring systems observed in the sample,
        and the other with frequencies of those ring systems in ChEMBL.
        Also the total number of molecules in the sample.
        """
        combined_sample_counts: defaultdict = defaultdict(int)
