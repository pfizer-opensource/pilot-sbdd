...
from .file_prep import FilePrep, get_files
...
def get_plifs(
    ...
    for target in posebuster_targets:
        try:
            ...
            warnings.warn(
                f"Could not prepare system for {method}::{target}", stacklevel=2
            )
            continue
        ...
        try:
            ...
            warnings.warn(
                f"Could not calculate PLIFs for {method}::{target}", stacklevel=2
            )
            continue
        plifs[target] = fp
    return plifs
...
def adjust_fingerprint_residues(fp: plf.Fingerprint, mapper: dict[str, int]) -> IFPType:  # type: ignore
...
) -> tuple[IFPType, IFPType]:  # type: ignore
    """
