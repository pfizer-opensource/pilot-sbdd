...
def main():
    ...
    train_sdf = list(train_dir.glob("*.sdf"))
    val_sdf = list(val_dir.glob("*.sdf"))
    test_sdf = list(test_dir.glob("*.sdf"))
    n_samples_after = {}
    ...
    failed_save = []
    for split_dir, split in zip(
        [test_dir, val_dir, train_dir], ["test", "val", "train"]
    ):

        sdf_files = list(split_dir.glob("*.sdf"))
        lig_coords = []
        lig_atom = []
        lig_mask = []
        lig_mol = []
        ...
        for sdffile in pbar:
            pdbfile = sdffile.parents[0] / (sdffile.stem.split("_")[0] + ".pdb")
            try:
                _ = PDBParser(QUIET=True).get_structure("", pdbfile)
            except Exception:
                num_failed += 1
                failed_save.append((pdbfile, sdffile))
                print(failed_save[-1])
                pbar.set_description(f"#failed: {num_failed}")
                continue
            try:
                ...
                print(type(e).__name__, e, pdbfile, sdffile)
                num_failed += 1
                pbar.set_description(f"#failed: {num_failed}")
                continue

            pocket_name = pdbfile.stem
            ligand_name = sdffile.stem
            ...
            count += 1
            pocket_id_fname = str(pdbfile).replace(".pdb", "_pocket_ids.txt")
            # specify pocket residues
            with open(pocket_id_fname, "w") as f:
    ...
    summary_string += f"num_samples train: {len(train_sdf)}\n"
    summary_string += f"num_samples val: {len(val_sdf)}\n"
    summary_string += f"num_samples test: {len(test_sdf)}\n\n"
